...
class BarterPromotions extends StatefulWidget {
  const BarterPromotions({super.key});

  @override
  State<BarterPromotions> createState() => _BarterPromotionsState();
}

class _BarterPromotionsState extends State<BarterPromotions> {
  ...
  void fetchPendingRequests() async {
    ...
    // Fetch all EventPromotion documents
    ...
    for (var event in allEvents) {
      ...
      requestFutures.add(
          _processPromotion(event, pendingRequests)
      );
    }
  ...
  Future<void> _processPromotion(QueryDocumentSnapshot event, List<Map<String, dynamic>> pendingRequests) async {
    ...
    if (event['collabType'] == 'promotor') {
      if (reqData.docs.isEmpty) return;

      var requestData = reqData.docs[0].data() as Map<String, dynamic>;
      if (requestData['isPaid'] != null && requestData['isPaid'] == true) return;
    }
    ...
    Map<String, dynamic> data = {
      ...
      'isPaid': reqData.docs.isEmpty ? false : (reqData.docs[0].data() as Map<String, dynamic>)['isPaid'] ?? false
    };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // appBar: appBar(context, title: "Barter"),
