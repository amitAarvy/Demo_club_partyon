...
class AddressInfluencer extends StatefulWidget {

  final String id;
  final Map<String, dynamic> data;


  const AddressInfluencer({super.key,
    required this.id, required this.data,});

  @override
  State<AddressInfluencer> createState() => _AddressInfluencerState();
}

class _AddressInfluencerState extends State<AddressInfluencer> with WidgetsBindingObserver {
              ...
              GestureDetector(
                onTap: () async {
                  try {
                    ...
                    print('wiget.id check influecner ${widget.data}');
                    await FirebaseFirestore.instance.collection("Influencer").doc(widget.id).set(widget.data).onError((error, stackTrace) async {
                      Fluttertoast.showToast(msg: 'Something went wrong');
                      await FirebaseFirestore.instance.collection("Influencer").doc(widget.id).delete().then((e){
                        Get.offAll(const LoginPage());
                      });
                    }).whenComplete(() async {
                      await ReferController.updateReferral(
                          "influencer", widget.data['companyMame']);
                      // await ReferController.updateReferral(
