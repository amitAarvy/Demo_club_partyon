...
Future<void> ProfileUploadImage(
    List<File> images, String eventID, HomeController homeController,
    {required List coverImages,
      bool isOffer = false,
      List<Uint8List> webImages = const [],
      bool isOrganiser = false}) async {

  var imageUrls;
  if(kIsWeb){
    imageUrls = await Future.wait(webImages.map((image) async {
      String url = '';
      if (isOffer) {
        url = 'Profile/$eventID/coverImage/coverImage${randomAlphaNumeric(4)}.jpg';
      } else {
        url = 'Profile/$eventID/offerImage/offerImage${randomAlphaNumeric(4)}.jpg';
      }
  ...
  if (isOffer) {
    FirebaseAuth.instance.currentUser
        ...
        .whenComplete(() {
      FirebaseFirestore.instance.collection("Organiser").doc(uid()).set(
          {'profileImages': images.isNotEmpty ? imageUrls : coverImages},
          SetOptions(merge: true));
    });
  }
}

Future<void> MenuUploadImage(
    List<File> images, String eventID, HomeController homeController,
    {required List coverImages,
      bool isOffer = false,
      bool isOrganiser = false}) async {
  var imageUrls = await Future.wait(images.map((image) async {
    String url = '';
    if (isOffer) {
      url = 'Menu/$eventID/coverImage/coverImage${randomAlphaNumeric(4)}.jpg';
    } else {
      url = 'Menu/$eventID/offerImage/offerImage${randomAlphaNumeric(4)}.jpg';
    }
    final Reference ref = FirebaseStorage.instance.ref().child(url);
    String downloadUrl = '';
    final UploadTask uploadTask = ref.putFile(image);
  ...
  if (isOffer) {
    FirebaseAuth.instance.currentUser
        ?.updatePhotoURL(images.isNotEmpty ? imageUrls[0] : coverImages[0])
        .whenComplete(() {
      FirebaseFirestore.instance.collection("Menu").doc(eventID).set(
          {'menuImages': images.isNotEmpty ? imageUrls : coverImages},
          SetOptions(merge: true));
    });
  }
}

Widget eventCarousel(List images, {bool isEdit = false, double? height, double aspectRatio = 16/9}) =>
          ...
          return Builder(
            builder: (BuildContext context) {
              return Container(
                ...
                child: lookupMimeType(image is File ? image.path : Uri.parse(image).path)!.contains('video')
                ? CustomVideoPlayer(link: image)
                : isEdit
                    ? CachedNetworkImage(
                  imageUrl: image,
...
class MenuEventController extends GetxController {
