...
class PromotionalAnalysisEventList extends StatefulWidget {
  final String eventPromotionId;
  final RetrieveAllContentItems? contentData;

  const PromotionalAnalysisEventList(
      {required this.eventPromotionId, Key? key, this.contentData})
      : super(key: key);

  @override
  State<PromotionalAnalysisEventList> createState() =>
      _PromotionalAnalysisEventListState();
}

class _PromotionalAnalysisEventListState
    extends State<PromotionalAnalysisEventList>
  ...
  void fetchPromotorData() async {
    ...
    for (var element in data.docs) {
      String influencerPromotorId = element['influencerPromotorId'];
      Map? influencerDataMap = await fetchInfluencerData(influencerPromotorId);
  ...
  Future<Map?> fetchInfluencerData(String influencerId) async {
    try {
      ...
      if (postsSnapshot.exists) {
        ...
        print("Document B does not exist.");
      }
    } catch (e) {
  ...
  Widget eventCard(List data, DateTime date,
    ...
    Map userData = data[index]['userData'] ?? {};
    return Column(
                ...
                Container(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Following Count : ${userData['followingCount'] ?? 0}',
                          // "Followers : ${data[index]['userData']['follower'] ?? 0}",
                          maxLines: 3,
                          overflow: TextOverflow.ellipsis,
                          style: GoogleFonts.ubuntu(color: Colors.white),
                                          ...
                                          onPressed: () async {
                                            try {
                                              phylloController.creatorProfile =
                                                  await PhylloController
                                                      .retrieveCreatorProfilesData(
                                                          userData['userName']);
                                              if (phylloController
                                                      .creatorProfile !=
                                                  null) {
                                                print(
                                                    'getting to creator view');
                                                Get.to(CreatorView(
                                                  userName: phylloController
                                                      .creatorProfile?.username,
                                                  imageURL: phylloController
                  ...
                  if ((isUpcoming
                    ...
                    ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return eventCard(promoterList!, date,
                              index: index,
                              isCancelled: false,
                              isActive: status);
                          // } catch (e) {
